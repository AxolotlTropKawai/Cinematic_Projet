...
public class control_V2 : MonoBehaviour {
    ...
    public float speed = 20f;                // Vitesse du joueur
    public float jump = 17f;                 // Hauteur du saut
    ...
    // La on déclare un RaycastHit, ça nous servira plus tard pour vérifier si on touche le sol
    ...
    void jumpCheck() {
        ...
        hit = Physics2D.Raycast(transform.position, Vector2.down, (monCollider.bounds.extents.y + Mathf.Abs(monCollider.offset.y) * transform.localScale.y) * longueurCheckJump);
        ...
        Debug.DrawRay(transform.position, Vector2.down * (monCollider.bounds.extents.y + Mathf.Abs(monCollider.offset.y) * transform.localScale.y) * longueurCheckJump, Color.red);
    ...
    private void OnTriggerEnter2D(Collider2D trucQuiMeChange)
    {
        if (trucQuiMeChange.tag == "Boost")
        {
            Destroy(trucQuiMeChange.gameObject);
            StartCoroutine(endBoost());
        }

        if (trucQuiMeChange.tag == "Deboost")
        {
            Destroy(trucQuiMeChange.gameObject);
            StartCoroutine(endDeboost());
        }
    }

    private void OnTriggerStay2D(Collider2D trucQuiMeRalentit)
    {
        if (trucQuiMeRalentit.gameObject.tag == "SlowZone")
        {
            speed = 10f;
            jump = 10f;
        }
    ...
    private void OnTriggerExit2D(Collider2D trucQuiMeRalentit)
    {
        if (trucQuiMeRalentit.gameObject.tag == "SlowZone")
        {
            speed = 20f;
